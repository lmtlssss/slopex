...
async function installSlopex({ postinstall = false } = {}) {
  ...
  if (!codexInstall) {
    ...
    if (postinstall) {
      console.warn(`[slopmodcodex] ${message}`);
      return;
    }
  ...
  if (!patchedBinary) {
    const message = [
      `No prebuilt slopmodcodex binary is available for Codex ${codexInstall.version} on ${artifactPlatformKey()}.`,
      "Install a slopex release that publishes this artifact, or place a matching binary under ~/.slopex/artifacts and rerun `slopmodcodex patch`."
    ].join(" ");
    if (postinstall) {
      console.warn(`[slopmodcodex] ${message}`);
      return;
  ...
  if (!fs.existsSync(backupBinary)) {
    if (originalBinarySha256 === patchedBinarySha256) {
      const message = [
        ...
        "Refusing to overwrite the only copy. Reinstall @openai/codex first, then rerun `slopmodcodex patch`."
      ].join(" ");
      if (postinstall) {
        console.warn(`[slopmodcodex] ${message}`);
        return;
  ...
  console.log(`slopmodcodex patched Codex ${codexInstall.version}`);
...
async function printStatus() {
  ...
  console.log(`supported by slopmodcodex: ${SUPPORTED_CODEX_VERSIONS.includes(codexInstall.version)}`);
  console.log(`slopmodcodex config block: ${managedConfigPresent ? "present" : "missing"}`);
  console.log(
  ...
  if (!state) {
    console.log("slopmodcodex install state: not found");
    return;
  }

  console.log(`slopmodcodex version: ${state.slopexVersion}`);
...
async function uninstallSlopex() {
  ...
  if (!state && !targetVersion) {
    throw new Error("No slopmodcodex install state was found and Codex is not installed globally. Nothing to uninstall.");
  }
...
function assertSupportedCodexVersion(version) {
  ...
  throw new Error(
    `Codex ${version} is not supported by this slopmodcodex release. Supported versions: ${SUPPORTED_CODEX_VERSIONS.join(", ")}`
  );
}
...
function releaseArtifactName(codexVersion) {
  return executableName(`slopmodcodex-codex-${codexVersion}-${artifactPlatformKey()}`);
}
