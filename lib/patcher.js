...
async function uninstallSlopex() {
  ...
  const codexInstall = resolveCodexInstall();
  if (!codexInstall) {
    throw new Error("Codex is not installed globally. Reinstall @openai/codex before running `slopex uninstall`.");
  }

  if (codexInstall.version !== state.codexVersion) {
    throw new Error(
      `Installed Codex is ${codexInstall.version}, but slopex backup is for ${state.codexVersion}. Refusing to overwrite a different Codex version.`
    );
  }

  if (codexInstall.vendorBinary !== state.vendorBinary) {
    throw new Error(
      "Codex vendor binary path changed since slopex was installed. Refusing to restore into an unexpected location."
    );
  }
  ...
  if (expectedPatchedSha256 && currentVendorSha256 !== expectedPatchedSha256) {
    throw new Error(
      "Current Codex binary no longer matches the slopex-installed binary. Refusing to overwrite unexpected changes."
    );
  }
  ...
  if (
    ...
    throw new Error("Stored Codex backup no longer matches the original binary hash. Refusing to restore a modified backup.");
  }
